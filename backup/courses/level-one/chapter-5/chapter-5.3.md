...
This if-then-else command is conditional logic. In Doc² workflows, conditional logic can be implemented with the [**IF node**](/workflow/integrations/core-nodes/workflow-nodes-base.if/), which splits a workflow conditionally based on comparison operations.

!!! note " IF vs Switch"
    If you need to filter data on more than two conditional routes that are possible with the *IF* node (true and false), use the [*Switch node*](/workflow/integrations/core-nodes/workflow-nodes-base.switch/). The *Switch node* is similar to the *IF* node, but supports up to four conditional routes.
