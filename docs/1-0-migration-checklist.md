...
If you have a custom nodes that were linked using `npm link` into the global `node_modules` directory, You'd need to link the again, but into `~/.n8n/nodes` instead.
