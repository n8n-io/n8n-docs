...
On activating this button, the [AI Agent](/glossary.md#ai-agent){ data-preview } will fill in the expression for you, with no need for any further user input.
...
You can see the `$fromAI()` function in action in the following [templates](/glossary.md#template-n8n){ data-preview }:
