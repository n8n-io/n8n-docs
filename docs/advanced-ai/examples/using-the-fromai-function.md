...
When configuring [app node](/integrations/builtin/app-nodes/index.md) [tools](/glossary/#ai-tool){ data-preview } connected to the Tools Agent, many parameters can be filled in by the AI model itself. The AI model will use the context from the task and information from other connected tools to fill in the appropriate details.
...
On activating this button, the [AI Agent](/glossary/#ai-agent){ data-preview } will fill in the expression for you, with no need for any further user input.
...
You can see the `$fromAI()` function in action in the following [templates](/glossary/#template-n8n){ data-preview }:
