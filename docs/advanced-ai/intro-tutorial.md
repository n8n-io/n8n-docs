...
An [AI agent](/glossary/#ai-agent){ data-preview } builds on [Large Language Models (LLMs)](/glossary/#large-language-model-llm){ data-preview }, which generate text based
on input by predicting the next word. While LLMs only process input to produce
output, AI agents add goal-oriented functionality. They can use [tools](/glossary/#ai-tool){ data-preview }, process
...
Every workflow needs somewhere to start. In n8n these are called ['trigger nodes'](/glossary/#trigger-node-n8n){ data-preview }. For this workflow, we want to start with a chat node.
...
1. Wait for the response, then type the message "What's my name?". The AI will not be able to tell you, however apologetic it may seem. The reason for this is we are not saving the context. The AI Agent has no [memory](/glossary/#ai-memory){ data-preview }.
