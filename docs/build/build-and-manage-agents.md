...
Agents are in Preview. They can make mistakes, and their behavior may change while the feature is in development. Knowledge bases on self-hosted are also in Preview.
{% endhint %}
...
Knowledge bases are available on n8n Cloud, and on self-hosted with a Daytona sandbox. They're in Preview on self-hosted. See [Self-hosted](build-and-manage-agents.md#self-hosted).
...
Agents run on self-hosted n8n, in Preview from n8n 2.32.3. There are two ways to set them up:
