...
- **What to do:** Migrate affected workflows to the current recommended alternatives before upgrading:
  ...
  - n8n automatically migrates existing **AI Transform** nodes to **Code** nodes on upgrade, keeping the same generated JavaScript, so existing workflows keep working without changes. You can no longer add an **AI Transform** node; write JavaScript directly in the **Code** node instead.
