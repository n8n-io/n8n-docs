...
1. In **Next URL**, use an [expression](/glossary.md#expression-n8n){ data-preview } to set the URL. The exact expression depends on the data returned by your API. For example, if the API includes a parameter called `next-page` in the response body:
	```javascript
