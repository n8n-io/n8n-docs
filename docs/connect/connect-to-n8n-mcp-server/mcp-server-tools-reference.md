...
`search_folders` is available from n8n 2.14.0. It's only registered when the instance is licensed for folders.
{% endhint %}
...
- When several folders match a name, use the `path` values to ask the user which one they meant.
...
`create_folder` is only registered when the instance is licensed for folders.
{% endhint %}
...
- When a folder ID can't be found, the error message points you at `search_folders` to look up a valid folder ID.
...
`update_folder` is only registered when the instance is licensed for folders.
{% endhint %}
...
| `parentFolderId` | `string` | No | New parent folder ID to move the folder under. It must belong to the same project and must not be a descendant of the folder being moved. Pass `"0"` to move the folder to the project root. Omit to leave the folder where it is. |
...
`move_workflows_to_folder` is only registered when the instance is licensed for folders.
{% endhint %}
...
| `failed` | `array` | Workflows that couldn't be moved, with the reason for each |
| `failed[].workflowId` | `string` | The ID of the workflow that couldn't be moved |
| `failed[].error` | `string` | Why the workflow couldn't be moved |
| `error` | `string` | Error message explaining why the move failed. Present only on failure |

#### Notes <a href="#notes" id="notes"></a>

- Requires the `workflow:update` permission on each workflow and the `folder:read` permission on the destination folder.
- Moves may partially succeed. Report any entries in `failed` to the user.
- If none of the workflows could be moved, the response returns the error `None of the workflows could be moved`.
- Archived workflows and workflows that aren't available to MCP tools can't be moved.
...
- If the user names a target folder, call `search_folders` first to resolve it.
- After creation, tell the user which project the workflow was created in using the `targetProject` field, and which folder using the `targetFolder` field, if any.
