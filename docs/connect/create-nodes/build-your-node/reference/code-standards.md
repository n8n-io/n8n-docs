...
Keep your node close to the API it wraps. Layers between the parameters a user fills in and the request n8n sends make the node harder to read and harder to fix when the API changes.
...
* Split your node across several files when its size makes a single file awkward, not by default. Refer to [Node file structure](../../plan-your-node/choose-node-file-structure.md) for more information.

Add an abstraction when you have repetition to remove, not in case you get some later.
