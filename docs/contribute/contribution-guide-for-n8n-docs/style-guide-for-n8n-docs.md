...
* **Every section landing page links down to its children.** With two or more children, list them under a heading like "In this section" as a plain bullet list (link plus a short description) rather than a GitBook content-ref card. With only one child, skip the list and link it inline at its first mention instead.
* **Every child page links back up and sideways, in a "Related pages" section at the end.** List a link back to the parent (with `./`), plus a link to each direct sibling in the same section. This is a plain link list, structural rather than curated — it doesn't replace whatever sibling links you've already worked into the body.
* **A page that is itself a parent does both.** A section landing page one level down (for example, a subsection README) gets its own "In this section" for its children, plus a "Related pages" footer relating it to its own parent and its own siblings at its level. Reach only one level up and one level sideways — don't chain further up to a grandparent section; that connectivity already exists transitively through the parent's own links.
* **Skip sibling links for flat reference collections** where every page follows the same one-fact template (for example, one page per expression data type). These pages still link back to the parent in "Related pages", just without a sibling list.
