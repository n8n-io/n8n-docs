...
=== "Python"
	...
	| `_("<node-name>").itemMatching(currentNodeInputIndex)` | Use instead of `_("<node-name>").item` in the Code node if you need to trace back from an input item. Refer to [Accessing linked items in the Code node](/data/data-mapping/itemmatching.md) for an example. | :white_check_mark: |
