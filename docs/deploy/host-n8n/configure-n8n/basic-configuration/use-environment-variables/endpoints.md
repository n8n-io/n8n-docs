...
| `N8N_METRICS_INCLUDE_SCHEDULER_METRICS` | Boolean | `false` | Whether to include metrics for the durable scheduler, such as queue depth, scheduling lag, dispatch throughput, retries, and dead-letters. Main instances only. For what each metric means, see [Durable scheduler observability](../../durable-scheduler.md#observability). |
| `N8N_METRICS_SCHEDULER_INTERVAL` | Integer | `20` | How often (in seconds) to refresh the queue-health gauges for the durable scheduler. See [Durable scheduler observability](../../durable-scheduler.md#observability). |
| `N8N_METRICS_INCLUDE_POLL_TRIGGER_METRICS` | Boolean | `false` | Whether to include metrics for trigger nodes that poll, such as poll duration, poll errors, overlapping polls, and cursor commits. Main instances only. For what each metric means, see [Poll trigger metrics](../../durable-scheduler.md#poll-trigger-metrics). |
| `N8N_METRICS_INCLUDE_SYSTEM_TASK_METRICS` | Boolean | `false` | Whether to include metrics for n8n's internal maintenance tasks, such as run duration, last success, skipped occurrences, and timer lag. Main instances only. For what each metric means, see [System task observability](../../system-tasks.md#observability). |
