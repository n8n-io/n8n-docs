...
| `N8N_EXTERNAL_SECRETS_CONNECT_TIMEOUT` | Number | `20` | Maximum time (in seconds) n8n waits for a secrets provider to connect. If the provider doesn't answer in time, n8n marks it as errored, retries the connection in the background with increasing delays, and startup continues without its secrets. Available from n8n 2.41.0. |
