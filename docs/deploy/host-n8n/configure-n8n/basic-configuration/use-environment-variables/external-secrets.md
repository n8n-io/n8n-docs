...
| `N8N_EXTERNAL_SECRETS_CONNECT_TIMEOUT` | Number | `20` | Maximum time (in seconds) n8n waits for a secrets provider to connect. If the provider doesn't answer in time, n8n marks it as errored, retries the connection in the background with increasing delays, and startup continues without its secrets. |
| `N8N_EXTERNAL_SECRETS_REFRESH_TIMEOUT` | Number | `20` | Maximum time (in seconds) n8n waits for a secrets provider to deliver its secrets, at startup and on each update interval. If the fetch takes longer, n8n stops waiting and the fetch keeps running in the background. When it completes successfully, n8n stores the secrets. At startup, workflows that use secrets from that provider fail until the first fetch completes. On an update interval, the previously fetched secrets stay available. Increase this value for providers with many secrets. For HashiCorp Vault and Infisical, n8n also cancels each single HTTP request after the larger of the two timeouts. |
