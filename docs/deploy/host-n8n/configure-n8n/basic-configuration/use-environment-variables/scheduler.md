...
| `N8N_SCHEDULER_SYSTEM_TASKS_ENABLED` | Boolean | `false` | Whether the durable scheduler also runs n8n's internal maintenance jobs, such as pruning old executions, compacting insights data, and renewing the license. When off, the leader main runs these jobs on its own timers. Requires `N8N_SCHEDULER_ENABLED`. Set the same value on every main, and give every main the same `N8N_SCHEDULER_ENABLED`. A main with either flag off removes the durable schedules for these jobs at startup. Available from n8n 2.40.0. See [System tasks](../../durable-scheduler.md#system-tasks). |
