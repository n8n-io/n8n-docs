...
| `N8N_AZURE_STORAGE_CUSTOM_ENDPOINTS_ENABLED` | Boolean | `false` | Set to `true` to let Azure Storage Shared Key credentials use a custom endpoint, such as a private endpoint or a custom domain. The Azure sovereign clouds are always available and don't need this setting. |
