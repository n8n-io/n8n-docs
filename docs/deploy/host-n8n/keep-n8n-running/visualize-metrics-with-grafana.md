...
n8n exposes run-duration, last-success, in-flight, skip, and timer-lag metrics for its own [system tasks](../configure-n8n/system-tasks.md), the maintenance jobs that prune and compact your data. Enable them with `N8N_METRICS_INCLUDE_SYSTEM_TASK_METRICS`, then see [System task observability](../configure-n8n/system-tasks.md#observability) for what each metric means and for queries to start from.
