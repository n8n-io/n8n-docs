...
To offer OAuth login to users, it's possible to overwrite [credentials](/glossary.md#credential-n8n){ data-preview } on a global basis. This credential data isn't visible to users but the backend uses it automatically.
...
| `N8N_TEMPLATES_ENABLED` | Boolean | `true` | Enable [workflow templates](/glossary.md#template-n8n){ data-preview } (true) or disable (false). |
