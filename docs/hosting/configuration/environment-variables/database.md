...
| `DB_PING_MAX_FAILURES_BEFORE_RECOVERY` | Number | `3` | How many pings in a row must fail before n8n decides the connection is lost and starts recovery. |
...
| `DB_CONNECTION_ACQUISITION_TIMEOUT_MS` | Number | `30000` | How long, in milliseconds, a query waits while recovery is in progress before failing fast with an error. Set to `0` to wait indefinitely. |
...
| `DB_POSTGRESDB_MAX_CONNECTION_LIFETIME_MS`<br>/`_FILE` | Number | `3600000` | How long, in milliseconds, n8n keeps a pooled connection before recycling it. Recycling old connections helps avoid stale connections that the database or network has silently dropped. The default is one hour. Set to `0` to disable recycling. |
...
n8n regularly checks that it can still reach your database. If the connection drops, for example after a database restart or a network blip, n8n can repair it on its own instead of staying broken until you restart n8n.
...
The health checks and connection recovery run for every database type. Some pool-level protections, such as the `DB_CONNECTION_ACQUISITION_TIMEOUT_MS` wait, only apply to PostgreSQL. The default values work well for most setups, so you usually don't need to change them.
///
