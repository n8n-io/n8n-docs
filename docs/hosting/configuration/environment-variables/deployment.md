...
| `N8N_DISABLE_UI` | Boolean | `false` | If set to `true`, disables the UI. |
| `N8N_PREVIEW_MODE` | Boolean | `false` | If set to `true`, n8n runs in preview mode. |
...
| `N8N_DEV_RELOAD` | Boolean | `false` | Whether the server should automatically reload or restart when changes occur in the source code. |
