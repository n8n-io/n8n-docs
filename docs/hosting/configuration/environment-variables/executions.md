...
| `EXECUTIONS_DATA_MAX_DISPLAY_SIZE` | Number | `104857600` | Maximum size (in bytes) of execution data that n8n loads when displaying an execution (editor, execution detail, and public API). Larger executions are returned without their data (shown as "too large to display") to avoid running low-resource instances out of memory. Doesn't affect retrying or resuming executions, which always load the full data. Set to `0` to disable the limit. |
...
| `N8N_CONCURRENCY_EVALUATION_LIMIT` | Number | License-tier default | Max test cases that can run in parallel within a single [evaluation](/advanced-ai/evaluations/metric-based-evaluations.md#run-test-cases-in-parallel) test run. When unset, the limit follows the license tier (Community/Pro: 1, Business: 3, Enterprise: 5). Setting this overrides the tier default. |
