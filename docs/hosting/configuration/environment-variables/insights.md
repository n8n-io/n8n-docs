...
`N8N_INSIGHTS_COMPACTION_HOURLY_TO_DAILY_THRESHOLD_DAYS` and `N8N_INSIGHTS_COMPACTION_DAILY_TO_WEEKLY_THRESHOLD_DAYS` set how many days of high-resolution detail n8n keeps before each rollup (hour bucket to day bucket, then day bucket to week bucket). You configure those day counts on your instance.
