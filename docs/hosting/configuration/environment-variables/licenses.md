...
| `N8N_LICENSE_DETACH_FLOATABLE_ON_SHUTDOWN` | Boolean | `true` | Controls whether floatable entitlements are released back to the pool upon shutdown. Set to `true` to allow reuse by other instances, or `false` to retain entitlements. <br> For production instances that should never lose licensed features, it is recommended to set this to `false`. |
