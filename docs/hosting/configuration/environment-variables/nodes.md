...
| `NODE_FUNCTION_ALLOW_EXTERNAL` | String | - | Permit users to import specific external modules (from `n8n/node_modules`) in the Code node. n8n disables importing modules by default. Note that you must specify the exact module names, separated by commas. Using `*` (wildcard) doesn't work for this variable. |
