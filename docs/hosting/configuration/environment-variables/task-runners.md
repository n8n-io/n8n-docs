...
| `N8N_BLOCK_RUNNER_ENV_ACCESS` | Boolean | `true` | Whether to block access to environment variables from within Python code tasks. Set to `false` to enable all Python code node users to access environment variables via `os.environ`. For security reasons, environment variable access is blocked by default. | 
