...
When `<AREA>_MANAGED_BY_ENV` is `false` (the default), n8n ignores the related environment variables, even if you set them. Setting it back to `false` after enabling it doesn't reset the stored values, it only restores normal UI behavior.
...
| Area | Activating variable | Reference |
| :----- | :---------- | :-------- |
| Instance owner | `N8N_INSTANCE_OWNER_MANAGED_BY_ENV` | [Instance owner via environment variables](/hosting/configuration/environment-variables/user-management-smtp-2fa.md#instance-owner-via-environment-variables) |
| SSO | `N8N_SSO_MANAGED_BY_ENV` | [SSO environment variables](/hosting/configuration/environment-variables/sso.md) |
| Security policy | `N8N_SECURITY_POLICY_MANAGED_BY_ENV` | [Security policy via environment variables](/hosting/configuration/environment-variables/security.md#security-policy-via-environment-variables) |
| Log streaming | `N8N_LOG_STREAMING_MANAGED_BY_ENV` | [Log streaming](/log-streaming.md#configure-via-environment-variables) |
...
* **Owner password must be a bcrypt hash.** `N8N_INSTANCE_OWNER_PASSWORD_HASH` expects a pre-hashed bcrypt value. Setting a plaintext password breaks login.
* **SAML metadata variables are mutually exclusive.** Set either `N8N_SSO_SAML_METADATA` (inline XML) or `N8N_SSO_SAML_METADATA_URL` (URL), not both.
* **UI controls are read-only by design.** If a setting is read-only and you didn't expect it, check whether the matching `*_MANAGED_BY_ENV` variable is `true` in your environment.
* **Values persist when you turn off `*_MANAGED_BY_ENV`.** Setting `*_MANAGED_BY_ENV` back to `false` restores UI write access but keeps the values that were last applied. Edit them through the UI afterward if you want to change them.
