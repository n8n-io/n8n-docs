...
Set a bucket-level time to live (TTL) so that S3 automatically deletes old binary data. n8n delegates pruning of binary data to S3, so setting a TTL is required unless you would like to preserve binary data indefinitely.
