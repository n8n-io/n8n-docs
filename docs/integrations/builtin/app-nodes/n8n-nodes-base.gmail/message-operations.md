...
* **Advanced Email Options**: Whether to set additional email fields such as CC, BCC, sender name, and reply threading. Refer to [Advanced email options](#advanced-email-options) below for more information.
...
The extra click protects the approval from automation. Many email security tools open every link in an incoming email to scan it, which can record a response before anyone reads the message. With the confirmation page on, opening the link only displays the page: n8n records a response only when a person presses the button.

A few details about how the page behaves:
...
* Once someone responds, the links stop working: anyone opening them later sees a page saying no further action is needed.
...
* **Reply in Thread**: The ID of a Gmail conversation to send the message in, instead of starting a new one. Useful for keeping recurring approvals in a single conversation. n8n takes the email subject from the thread, so the **Subject** field is ignored.
...
All recipients — including CC and BCC — receive the same response links. The workflow resumes with the first response; anyone responding later sees a page saying no further action is needed.
{% endhint %}
...
* **No record of who responded**: An email link carries no identity, so the node's output tells you what was decided, not who decided it.
