...
Adding the Microsoft SharePoint node to a workflow creates version 2 of the node, which uses the Microsoft Graph API. Workflows built with version 1 stay on version 1 and keep working unchanged, with the credential they already use. Refer to [What changed from version 1](#what-changed-from-version-1).
...
* **Site search fails with per-site permissions.** Microsoft Graph can't list sites for a credential that only has `Sites.Selected`. Switch the **Site** field to **By URL** or **By ID** mode, or grant a tenant-wide read permission if you need search.
