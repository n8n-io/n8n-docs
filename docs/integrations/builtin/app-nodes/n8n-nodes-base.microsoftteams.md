...
Refer to [Microsoft credentials](../credentials/microsoft.md) for guidance on setting up authentication. Support for [Microsoft Entra Service Principal credentials](../credentials/microsoftentraserviceprincipal.md), for app-only access with no signed-in user, is available from n8n version 2 or later. Select **Service Principal (App-Only)** in the **Authentication** dropdown.
{% endhint %}
