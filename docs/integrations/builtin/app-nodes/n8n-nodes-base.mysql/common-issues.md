...
description: >-
  Documentation for common issues and questions in the MySQL node in n8n, a
...
### If only MySQL is in Docker <a href="#if-only-mysql-is-in-docker" id="if-only-mysql-is-in-docker"></a>
...
When configuring [MySQL credentials](../../credentials/mysql.md), the `localhost` address should work without a problem (set the **Host** to `localhost`).
...
When configuring [MySQL credentials](../../credentials/mysql.md), use `host.docker.internal` as the **Host** address instead of `localhost`.
...
When configuring [MySQL credentials](../../credentials/mysql.md), use the MySQL container's name as the host address instead of `localhost`. For example, if you call the MySQL container `my-mysql`, you would set the **Host** to `my-mysql`.
...
If MySQL and n8n are running in the same Docker container, the `localhost` address doesn't need any special configuration. You can configure MySQL to listen on `localhost` and configure the **Host** in the [MySQL credentials in n8n](../../credentials/ollama.md) to use `localhost`.
