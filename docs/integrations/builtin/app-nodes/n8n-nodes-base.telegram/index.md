...
You can only use `@channelusername` on public channels. To interact with a Telegram group, you need the ID of that group.
...
2. From your web browser: Open Telegram in a web browser and open the group chat. The group chat ID is the series of digits behind the letter "g". Prefix your group Chat ID with a `-`.
3. Invite Telegram's [@RawDataBot](https://t.me/RawDataBot){:target=_blank .external-link} to the group chat: Once you add it, the bot outputs a JSON file that includes a `chat` object. The `id` for that object is the Group ID. Then remove the RawDataBot from your channel.
