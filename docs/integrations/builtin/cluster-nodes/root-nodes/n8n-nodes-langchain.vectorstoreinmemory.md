...
Use the In Memory Vector Store node to store and retrieve [embeddings](/glossary.md#ai-embedding){ data-preview } in n8n's in-app memory. 
...
This node creates a [vector database](/glossary.md#ai-vector-store){ data-preview } in the app memory.
///
...
You can connect the In-Memory Vector Store node directly to the [tool](/glossary.md#ai-tool){ data-preview } connector of an [AI agent](/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/index.md) to use vector store as a resource when answering queries.
