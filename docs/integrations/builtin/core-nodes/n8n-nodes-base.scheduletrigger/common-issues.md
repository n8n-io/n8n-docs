...
You can update the scheduled trigger interval at any time but it only gets updated when the workflow is published. If you change the trigger interval after the workflow is published, the changes won't take effect until you stop and then publish a new version of the workflow.
