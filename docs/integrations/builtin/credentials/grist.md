...
4. Select these scopes: `offline_access`, `doc:read`, and `doc:write`.
...
{% hint style="info" %}
**Register the app with every scope the credential requests**

Grist refuses an authorization that asks for a scope the app wasn't registered with, and connecting fails with an `invalid_scope` error. If you register an app with fewer scopes, use [custom scopes](#setting-custom-scopes) to request only those.
{% endhint %}
...
Grist OAuth2 credentials use the following scopes by default:

* `offline_access`
* `doc:read`
* `doc:write`

To select different scopes for your credentials, enable the **Custom Scopes** slider and edit the **Enabled Scopes** list. Keep in mind that some features may not work as expected with more restrictive scopes. For example, a credential without `doc:write` can read records but can't create, update, or delete them. Refer to [Grist's OAuth scopes](https://support.getgrist.com/oauth-apps/#scopes) for the full list of available scopes.
