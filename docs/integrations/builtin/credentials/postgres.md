...
7.  Enter the **Port** number to use for the connection. You can either run the `/conninfo` command to confirm the host name or run this query:
    ...
    ```
8. **SSH Tunnel**: Turn this setting on to connect to the database over SSH. Refer to [SSH tunnel limitations](postgres.md#ssh-tunnel-limitations) for some guidance around using SSH. Once turned on, you'll need:
