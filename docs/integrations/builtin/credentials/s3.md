...
description: >-
  Documentation for S3 credentials. Use these credentials to authenticate S3 in
...
More detailed instructions for DigitalOcean Spaces, Tigris, Wasabi, and RustFS follow. If you're using a different provider, refer to their documentation for more information.
...
To configure the credential for use with [RustFS](https://rustfs.com/):

1. For the **S3 Endpoint**, enter your RustFS server URL with the S3 API port (default is 9000). For example: `http://your-rustfs-server:9000`
    - **Note**: RustFS uses port 9000 for the S3 API, not port 9001 (which is the web console).
2. For the **Region**, enter `us-east-1` (or your configured region).
3. Access your RustFS web console (default port 9001) and log in.
4. Navigate to **Access Keys** or **IAM** settings.
5. Create a new access key or copy an existing one.
6. Enter the **Access Key** as the **Access Key ID** in n8n.
7. Enter the **Secret Key** as the **Secret Access Key** in n8n.
8. Turn on the **Force Path Style** toggle. RustFS uses path-style addressing for S3 API requests.
9. Decide how you want the n8n credential to handle SSL:
    - If your RustFS instance uses HTTPS with valid certificates, keep **Ignore SSL Issues** turned off.
    - If your RustFS instance uses HTTP or self-signed certificates, turn on **Ignore SSL Issues**.
