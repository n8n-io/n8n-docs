---
title: LoneScale Trigger node documentation
description: >-
  Learn how to use the LoneScale Trigger node in n8n. Follow technical
  documentation to integrate LoneScale Trigger node into your workflows.
contentType:
  - integration
  - reference
nodeTitle: LoneScale Trigger node documentation
originalFilePath: integrations/builtin/trigger-nodes/n8n-nodes-base.lonescaletrigger.md
originalUrl: >-
  https://docs.n8n.io/integrations/builtin/trigger-nodes/n8n-nodes-base.lonescaletrigger
url: >-
  https://docs.n8n.io/integrations/builtin/trigger-nodes/n8n-nodes-base.lonescaletrigger
layout:
  description:
    visible: false
---

# LoneScale Trigger node <a href="#lonescale-trigger-node" id="lonescale-trigger-node"></a>

Use the LoneScale Trigger node to respond to workflow events in [LoneScale](https://www.lonescale.com/) and integrate LoneScale with other applications.

On this page, you'll find a list of operations the LoneScale node supports, and links to more resources.

{% hint style="info" %}
**Credentials**

Refer to the [LoneScale credentials documentation](../credentials/lonescale.md) for authentication information for this node.
{% endhint %}

{% hint style="info" %}
**Examples and templates**

For usage examples and templates to help you get started, refer to n8n's [LoneScale Trigger integrations](https://n8n.io/integrations/lonescale-trigger/) page.
{% endhint %}

## Events <a href="#events" id="events"></a>

- On new LoneScale event

## Related resources <a href="#related-resources" id="related-resources"></a>

n8n provides an app node for LoneScale. Refer to the [LoneScale node documentation](../app-nodes/n8n-nodes-base.lonescale.md) for more information.

View [example workflows and related content](https://n8n.io/integrations/lonescale-trigger/) on n8n's website.
