...
* **Emoji Names to Filter**: Enter a comma-separated list of emoji names (without colons, for example `thumbsup,eyes,white_check_mark`) to limit **Reaction Added** events to only those specific reactions. n8n applies this filter at the trigger level before workflow execution starts. Leave empty to trigger on any reaction.
