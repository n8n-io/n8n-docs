...
16. Click on the ***Sign in with Google*** button in the OAuth section to connect a Google account to n8n.
...
If you use the OAuth2 authentication method, you have to grant permission to access the API. Refer to the [Using OAuth 2.0 to Access Google APIs](https://developers.google.com/identity/protocols/oauth2) documentation to learn more.
...
### How to resolve the "Google hasn't verified this app" warning?

If you're using the OAuth authentication method, you might come across the warning "Google hasn't verified this app". This is a security measure taken by Google. To avoid this, you can create the OAuth credentials from the same account you want to authenticate. However, if you're using credentials that were generated by another account (by a developer, or another third party), you can follow the steps mentioned below to authenticate
