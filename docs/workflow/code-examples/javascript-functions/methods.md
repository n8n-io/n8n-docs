...
This method allows you to return an item at a specific index. The index is zero-based. Hence, `$item(0)` will return the first item, `$item(1)` the second one, and so on. Refer to [this](/workflow/integrations/core-nodes/workflow-nodes-base.function/) documentation to learn more.
