...
This if-then-else command is conditional logic. In WF² workflows, conditional logic can be implemented with the [**IF node**](/integrations/core-nodes/WF²-nodes-base.if/){:target="_blank" .external}, which splits a workflow conditionally based on comparison operations.

!!! note "IF vs Switch"
    If you need to filter data on more than two conditional routes that are possible with the *IF* node (true and false), use the [*Switch node*](/integrations/core-nodes/WF²-nodes-base.switch/){:target="_blank" .external}. The *Switch node* is similar to the *IF* node, but supports up to four conditional routes.
...
To select this value, click on the wheel icon “Add Expression” on the right side of the Value 1 field.
    ...
    !!! note "Expressions"
        An expression is a string of characters and symbols in a programming language that represents a value depending upon its input. In WF² workflows, you can use expressions in a node to refer to another node for input data. In our example, the IF node references the data output by the HTTP Request node.
