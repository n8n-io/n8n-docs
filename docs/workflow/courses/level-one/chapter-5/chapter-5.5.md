...
In this step of the workflow you will learn how WF² data is structured and how to add custom JavaScript code to perform calculations using the *Function* node.
...
!!! warning "Function vs Function Item"
    WF² also provides a **Function Item node**, which should not be confused with the Function node. The Function Item node is used to add custom snippets of JavaScript code that should be executed once for every item that it receives as the input. Learn more about the difference between the Function and Function Item nodes [here](/data/code/){:target="_blank" .external}.


Before going into the setup of the Function node, you should first learn the [data structure](/data/data-structure/){:target="_blank" .external} of WF². This is important if you want to:
...
In WF², the data that is passed between nodes is an array of objects with the following structure:
...
[
    {
   	 ...
   	 json: { // The actual data WF² operates on (required)
   	 ...
   	 // Binary data of item. The most items in WF² do not contain any (optional)
   	 binary: {
...
**You 👩‍🔧**: There's an WF² node for that – I'll set it up in the next step.
