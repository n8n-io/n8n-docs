...
WF² uses the .env file to control how Doc² works. You can set items such as username and password for the editor or your timezone using this file. See the [WF² docs](/hosting/configuration/#configuration-via-file) for more details.
...
WF² uses certificates to keep user information safe when working in the [Editor UI](#editor-ui). You can also [customize the certificate](/hosting/configuration/) that you use in Workflow².
...
WF² [stores encrypted credentials](/workflow/integrations/credentials/) so that it can automate tasks that require this information to run properly.
...
WF² has created a [docker image](/hosting/installation/docker/) so that users who have a docker environment can quickly install and run Workflow².
...
WF² will sometimes use encryption keys for accessing [APIs](#API) when required. It also uses a [personal encryption key](/workflow/integrations/#credentials) to secure credentials.
...
Environment variables are predetermined values that typically get used to provide the ability to configure your program from outside of your application. An environment variable consists of a key-value pair. For example, `WF²_BASIC_AUTH_USER=user`. Here, `WF²_BASIC_AUTH_USER` is the key and `user` is the value.

WF² uses environment variables that help you configure your self-hosted Doc² instance. You can use environment variables to set [username](#username) and [password](#password) for your Doc² instance, configure the timezone, and a lot more. Refer to the [configuration](/hosting/configuration/) page to learn more about the environment variables used in Workflow².

## Execution
A completed run of a [workflow](#Workflow) from start to finish.

WF² logs workflow executions and allows the user to see if the workflow completed successfully or not.

WF² also has the ability to execute one workflow from another workflow.
...
WF² uses [expressions](/code-examples/expressions/) extensively when a [node](#Node) is referring to another node for input.
...
WF² is licensed under the fair-code model. See [faircode.io](https://faircode.io/) for more details.
...
WF² will often refer to [IP addresses](/hosting/configuration/) when accessing information on a system other than itself. This is more common when the service is on the local network rather than on the internet.
...
While Doc² is written in TypeScript, the final code generated is JavaScript and the [Function node](/workflow/integrations/core-nodes/workflow-nodes-base.function/) uses JavaScript to create customized [nodes](#Node). JavaScript is used in the WF²'s Function nodes.
...
WF² has the ability to send custom header information to many [APIs](#API), specifically in the [HTTP Request node](/workflow/integrations/core-nodes/workflow-nodes-base.httprequest/).
...
WF² runs on top of NodeJS and uses its libraries extensively.
...
WF² is [installed](/hosting/installation/npm/) and updated using npm.
...
WF² is also distributed as an [npm](#npm) package.
...
WF² also stores passwords as a part of [credentials](#Credentials) for some services.
...
WF² uses SSL certificates to secure the Doc² [UI](#UI) so that any information that is passed between the Doc² server and the user's web browser is encrypted and cannot be spied upon when it is travelling between the two. It is possible to [customize](/hosting/configuration/) the SSL installation.
...
WF² uses tokens often when requesting or sending data using [APIs](#API) or [webhooks](#Webhook).
...
WF² [uses a tunnel](/hosting/installation/npm/#WF²-with-tunnel) to allow external users to access Doc² installations that are behind firewalls, typically for triggering [webhooks](#Webhook). This setup is recommended for testing use only.
...
WF² has a web based UI, meaning that you program and interact with Doc² using a web browser.

Learn more at [user interface (UI)](https://searchapparchitecture.techtarget.com/definition/user-interface-UI).

## URL
An acronym for *Universal Resource Locator*, it is a string of characters that represents a network location. Most people's experience with URLs is in the form of a website address (e.g. https://WF².io)

WF²'s [Editor UI](#editor-ui) is accessed via an URL entered into a web browser (often https://localhost:5678).
...
WF² also stores usernames as a part of [credentials](#Credentials) for some services.
...
WF² uses variables extensively throughout its programming and nodes.
...
WF² has the ability to create [webhooks](/workflow/integrations/core-nodes/workflow-nodes-base.webhook/) that can be used to start workflows and receive data from systems outside of the Doc² system itself.
...
WF²'s primary form of programming uses a [workflow model](/workflows/workflows/) to represent both code and data flow.
