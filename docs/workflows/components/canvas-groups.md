...
![A workflow with several collapsed Canvas Groups](/_images/workflows/components/canvas-groups/canvas-groups-overview.png)
...
2. Select the **Group nodes** icon <span class="n8n-inline-image">![Group nodes icon](/_images/workflows/components/canvas-groups/group.svg){.off-glb}</span> in the toolbar above the selection, or press `Ctrl/Cmd` + `G`.
...
To break a Canvas Group back into separate nodes, select the **Ungroup** icon <span class="n8n-inline-image">![Ungroup icon](/_images/workflows/components/canvas-groups/ungroup.svg){.off-glb}</span> above it, or press `Ctrl/Cmd` + `Shift` + `G`. The nodes stay on the canvas.

## What you can group

Not every selection can become a Canvas Group. When you select nodes, n8n checks a few rules and only displays the **Group nodes** icon <span class="n8n-inline-image">![Group nodes icon](/_images/workflows/components/canvas-groups/group.svg){.off-glb}</span> when they all pass. If the icon doesn't appear, check your selection against these rules:
